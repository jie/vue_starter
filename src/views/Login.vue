...
<div class="min-h-screen flex items-center justify-center bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
  <div class="max-w-md w-full space-y-8 border-solid border p-5 rounded-3xl shadow-xl bg-gray-200">
    <div>
      <img class="mx-auto h-12 w-auto" src="../assets/helloSaas.png" alt="Workflow">
    ...
    <div class="mt-8 space-y-6" method="POST">
      ...
      <div class="rounded-md shadow-sm -space-y-px">
        <div>
          <label for="email-address" class="sr-only">{{ $t('base.email')}}</label>
          <input id="email-address" name="email" type="email" autocomplete="email" required class="appearance-none rounded-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-t-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm" :placeholder="$t('base.email')">
        </div>
        <div>
          <label for="password" class="sr-only">{{ $t('base.password')}}</label>
...
export default {
  ...
  async created() {
    await this.$store.dispatch("user/getCaptchaToken")
  },
  methods: {
    async login() {
